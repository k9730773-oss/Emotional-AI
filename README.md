...
使用 MTCNN 找出角色臉部位置，再用 DeepFace 進行角色辨識與情緒分析。
輸出每個影格的臉部表情（Neutral/Sad/Happy/Angry）。

speech_processing/ — 語音轉文字
使用 Whisper 將電影對白轉成文字，並生成 LRC/TXT/Excel 格式。
提供對話與時間軸資訊。
...
使用 RoBERTa 與 DeBERTa-v3-large 進行微調，輸入包含：
台詞內容
角色原始情緒（emotion1）
記憶類型
模型預測：
